...
| `list_collections` | Коллекции с организацией, числом элементов и доступом, по желанию одной организации; в режиме admin — участники |
...
| `list_organizations` | Admin: организации аккаунта, его роль, какими управляет инстанс |
...
Admin-тулзы принимают аргумент `organization`, когда инстанс управляет несколькими организациями.

У режима server свой набор тулзов:

| Тулза | Назначение |
|---|---|
| `get_status` | Сервер, число аккаунтов и организаций, включённые возможности |
| `list_users` | Аккаунты: статус, двухэтапный вход, последняя активность, организации |
| `get_user` | Один аккаунт по email или id |
| `list_organizations` | Организации сервера с подтверждёнными участниками и владельцами |
| `invite_user` | Приглашённый аккаунт для адреса: человек регистрируется при закрытой регистрации |
| `change_user` | Отключить, включить, завершить все сессии, повторить приглашение |
| `delete_user` | Удалить аккаунт; два вызова с кодом подтверждения, последнему подтверждённому владельцу или участнику организации — отказ |
| `delete_organization` | Удалить организацию со всем содержимым; два вызова с кодом подтверждения |

Панель администратора сообщает только подтверждённые членства: организация без подтверждённых участников в списке не видна и удаляется по id.
...
Процесс режима consumer или admin обслуживает один аккаунт Vaultwarden; членство аккаунта — граница того, до чего он дотягивается. Несколько агентов могут делить инстанс со своими bearer-токенами, каждый при желании сужен до чтения или до части коллекций.

| Режим | Действует как | Что делает |
|---|---|---|
| `consumer` | аккаунт | элементы его коллекций |
| `admin` | аккаунт владельца или администратора | ещё участники, коллекции и события его организаций; `VWMCP_ORGANIZATION` сужает, каких |
| `server` | admin-токен | аккаунты и организации всего сервера через панель администратора; содержимого сейфа не видит |

Инстанс режима server держит ключ ко всем аккаунтам сервера; его запускают отдельно от инстансов сейфа, со своими клиентами.
...
- Аккаунт с персональным API-ключом (Settings → Security → Keys → API key) и его мастер-пароль; для режима server — вместо них `ADMIN_TOKEN` сервера
...
| `VWMCP_CLIENT_ID` | Да, кроме режима server | — | client id API-ключа (`user.<uuid>`) |
| `VWMCP_CLIENT_SECRET` | Да, кроме режима server | — | client secret API-ключа |
| `VWMCP_MASTER_PASSWORD` | Да, кроме режима server | — | Мастер-пароль |
| `VWMCP_ADMIN_TOKEN` | Только режим server | — | `ADMIN_TOKEN` сервера в открытом виде |
| `VWMCP_CLIENTS` | Да (http) | — | `name:sha256(token)[:read_only][:collections=a\|b]` через запятую |
| `VWMCP_MODE` | Нет | `consumer` | `consumer`, `admin` или `server` |
| `VWMCP_ORGANIZATION` | Нет | все | Организации, которыми управляет режим admin: точные имена или id через запятую |
| `VWMCP_ALLOW_WRITE` | Нет | `false` | Создание, изменение, удаление элементов; изменения в admin; в режиме server — приглашения и изменения аккаунтов |
| `VWMCP_ALLOW_SHARE` | Нет | `false` | `share_with_human` (вместе с write) |
| `VWMCP_ALLOW_REVEAL` | Нет | `false` | `get_secret`, `get_attachment` |
| `VWMCP_ALLOW_PERMANENT_DELETE` | Нет | `false` | Удаление мимо корзины; в режиме server — удаление аккаунтов и организаций |
| `VWMCP_ALLOW_ADMIN_ROLES` | Нет | `false` | Выдавать и менять роли owner и admin |
| `VWMCP_INVITE_DOMAINS` | Нет | любые | Домены, адреса из которых могут приглашать `invite_member` и `invite_user` |
...
| `/health` | Процесс жив |
| `/ready` | Вход выполнен, последняя синхронизация успешна; в режиме server — последняя проверка панели администратора |
