...
// admin returns the management view for an unrestricted, writable client.
// Organization management is all-or-nothing: a client narrowed to some
// collections must not change who sees the others.
func (c *call) admin(ctx context.Context, write bool) (*vault.Admin, error) {
	...
	return c.server.Vault.Admin(ctx, c.server.Config.Organization)
}
...
func (s *server) registerAdminRead(srv *mcp.Server) {
	...
	addTool(s, srv, &mcp.Tool{
	...
	}, func(ctx context.Context, c *call, _ struct{}) (membersOutput, error) {
		admin, err := c.admin(ctx, false)
		if err != nil {
		...
		out := membersOutput{Members: []memberView{}}
...
func (s *server) registerAdminWrite(srv *mcp.Server) {
	...
	addTool(s, srv, &mcp.Tool{
		...
		if _, err := c.admin(ctx, true); err != nil {
